...
void HcaluLUTTPGCoder::init(const HcalTopology* top, const HcalElectronicsMap* emap, const HcalTimeSlew* delay) {
  ...
  sizeZDC_ = 2 * 9;
  size_t nluts = (size_t)(sizeHB_ + sizeHF_ + sizeZDC_ * 2 + 1);
...
void HcaluLUTTPGCoder::update(const HcalDbService& conditions) {
  ...
  for (std::vector<HcalElectronicsId>::const_iterator eId = vIds.begin(); eId != vIds.end(); eId++) {
    // The first HB is enough to tell whether to use new scheme in HB
    if (foundHB)
      break;
  ...
  for (const auto& id : metadata->getAllChannels()) {
    if (id.det() == DetId::Hcal and topo_->valid(id)) {
      ...
      const size_t SIZE = UPGRADE_LUT_SIZE;
      const int MASK = qieType == QIE10 ? QIE10_LUT_BITMASK : QIE11_LUT_BITMASK;
      double linearLSB = linearLSB_QIE11_;
          ...
          if (isMasked)
            ...
            if (is2018OrLater) {
              ...
              if (qieType == QIE11) {
                ...
                if ((((contain1TSHB_ and overrideDBweightsAndFilterHB_) or newHBtp) and cell.ietaAbs() <= lastHBRing)) {
